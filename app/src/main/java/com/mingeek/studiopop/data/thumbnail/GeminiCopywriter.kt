...
/**
 * Google Gemini API 로 썸네일 카피 후보 생성.
 *
 * 무료 tier 전제:
 * 1) 모델은 [ListModels 엔드포인트][1] 로 **동적 발견** — 하드코딩하지 않음. 새 모델 추가·폐지·
 *    권한 변화에 자동 적응.
 * 2) 품질 heuristic 으로 정렬(버전 desc → pro/flash/flash-lite → stable>preview).
 * 3) 각 모델 호출 실패(429/503/400-quota, 빈 응답, 파싱 오류 등) 는 Retryable 로 처리해
 *    쿨다운 건 뒤 **다음 모델로 폴백** — 전체 체인 끊기지 않음.
 * 4) 세션 메모이즈: 한번 막힌 모델은 쿨다운 동안 재시도 안 해 무료 tier quota 낭비 방지.
 *
 * [1]: https://generativelanguage.googleapis.com/v1beta/models
 */
class GeminiCopywriter(
    ...
    suspend fun suggestThumbnailCopies(
        ...
        runCatching {
            ...
            val prompt = buildPrompt(topic, tone, count)
            ...
            for (model in models) {
                ...
                when (val res = callModel(model, apiKey, prompt, count)) {
                    is ModelResult.Success -> return@runCatching res.copies
    ...
    /**
     * 1시간 캐시된 "품질 내림차순" 모델 리스트. 키가 바뀌면 자동 재조회.
     */
    ...
    /**
     * 품질 heuristic 내림차순 정렬:
     * 1) 버전 desc (2.5 > 2.0 > 1.5)
     * 2) 티어 desc (pro > flash > flash-lite)
     * 3) 안정성 asc (stable 가 preview/exp 보다 앞)
     * 4) 이름 asc (tie-break, 결과 결정론적)
     */
    ...
    /** 낮을수록 안정적: stable=0, preview/exp=1 */
    ...
    private sealed class ModelResult {
        data class Success(val copies: List<String>) : ModelResult()
    ...
    private fun callModel(
        ...
        val body = GenerateContentRequest(
            ...
            generationConfig = GenerationConfig(maxOutputTokens = 600, temperature = 0.9f),
        )
        ...
        return try {
            client.newCall(request).execute().use { response ->
                ...
                if (!response.isSuccessful) {
                    return@use mapErrorToRetryable(
                        code = response.code,
                ...
                val text = parsed.candidates
                    ...
                    .orEmpty()
                val copies = text.lines()
                    .map { it.trim().removePrefix("-").removePrefix("•").trim() }
                    .filter { it.isNotEmpty() }
                    .take(count)
                if (copies.isEmpty()) {
                    // safety block / MAX_TOKENS (thinking 토큰 소진 등) 로 본문 비어있음
                    ModelResult.Retryable("빈 응답(safety 또는 token 소진)", SHORT_COOLDOWN_MS)
                } else {
                    ModelResult.Success(copies)
                }
            }
        } catch (e: Exception) {
    ...
    /**
     * HTTP 에러 → Retryable. Retry-After 헤더를 존중하고, 없으면:
     * - 429 또는 본문에 RESOURCE_EXHAUSTED/quota 힌트 → LONG_COOLDOWN (RPD 초과 가능성)
     * - 404 → LONG_COOLDOWN (모델 권한 없음 — 세션 내 재시도 낭비)
     * - 503 기타 5xx → SHORT_COOLDOWN (transient)
     * - 그 외 → SHORT_COOLDOWN
     */
    private fun mapErrorToRetryable(
        code: Int,
        retryAfter: String?,
        body: String,
        ...
        val reasonTag = when {
            quotaHint -> "HTTP $code (quota)"
            else -> "HTTP $code"
        }
        return ModelResult.Retryable(reason = reasonTag, cooldownMs = cooldownMs)
    }

    private fun buildPrompt(topic: String, tone: String, count: Int): String =
        """
            당신은 한국 유튜브 썸네일 카피라이터입니다.
            짧고(8~18자) 강렬하며 클릭을 유도하는 카피를 작성합니다. 과도한 선정성/낚시성은 피하세요.
    ...
    companion object {
        ...
        /** transient 오류 (503/파싱 실패/빈 응답) 후 쿨다운. */
        private const val SHORT_COOLDOWN_MS = 30_000L

        /** 429 / quota / 404 등 "곧 다시 된다는 보장 없음" 오류 쿨다운. */
        private const val LONG_COOLDOWN_MS = 10 * 60_000L

        /** 모델 리스트 캐시 TTL (ListModels 재요청 주기). */
